...
class EF_Activation_Update_API_Check {
    ...
    public function update_check($transient) {
        ...
        if(!empty($transient->response[$this->plugin_name]['new_version'])){
            add_action('admin_notices', array(&$this, 'new_update_available_update_notice'));
        }
    ...
    public function request($false, $action, $args) {
        ...
        if (isset($response->errors['no_key']) && $response->errors['no_key'] == 'no_key' && isset($response->errors['no_subscription']) && $response->errors['no_subscription'] == 'no_subscription') {
            ...
            add_action('admin_notices', array(&$this, 'switched_subscription_error_notice'));
        }else if (!isset($response->errors) && !empty($response->new_version)) {
    ...
    /**
     * Display new update available notice
     * @param  string $message
     * @return void
     */
    public function new_update_available_update_notice($message) {

        $plugins = get_plugins();

        $plugin_name = isset($plugins[$this->plugin_name]) ? $plugins[$this->plugin_name]['Name'] : $this->plugin_name;

        echo sprintf('<div id="message" class="updated"><p>' . __('A new <strong>%s</strong> update is available.', $this->text_domain) . '</p></div>', ucfirst($plugin_name));
    }
