...
class InstagramAPI {
   ...
   * @return mixed
   */
  public function getTagMedia($name, $limit = 0, $auth = false ) {
    return $this->_makeCall('tags/' . $name . '/media/recent', $auth , array('count' => $limit));
  }
  ...
  public function pagination($obj, $limit = 0) {
    if (true === is_object($obj) && !is_null($obj->pagination)) {
      ...
      if (isset($obj->pagination->min_tag_id)) {
        return $this->_makeCall($function, $auth, array('max_id' => $obj->pagination->min_tag_id, 'count' => $limit));
      } else {
