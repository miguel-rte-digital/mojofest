    ...
    class WXR_Parser {
        ...
        function parse($file) {
            ...
            if (isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG) {
                ...
                echo '</pre>';
                echo '<p><strong>' . __('There was an error when reading this WXR file', 'dxef') . '</strong><br />';
                echo __('Details are shown above. The importer will now try again with a different parser...', 'dxef') . '</p>';
            }
    ...
    class WXR_Parser_SimpleXML {
        ...
        function parse($file) {
            ...
            if (!$success || isset($dom->doctype)) {
                return new WP_Error('SimpleXML_parse_error', __('There was an error when reading this WXR file', 'dxef'), libxml_get_errors());
            }
            ...
            if (!$wxr_version)
                return new WP_Error('WXR_parse_error', __('This does not appear to be a WXR file, missing/invalid WXR version number', 'dxef'));
...
if (!class_exists('WXR_Parser_Regex')) {
    ...
    class WXR_Parser_Regex {
        ...
        function parse($file) {
            $wxr_version = $in_post = $in_option = $in_term = false;
            ...
            if ($fp) {
                while (!$this->feof($fp)) {
                    ...
                    if (false !== strpos($importline, '</wp:option>')) {
                        $in_option = false;
                        $this->options[] = $this->process_option($option);
                        continue;
                    }
                    if ($in_option) {
                        $option .= $importline . "\n";
                    ...
                    if (false !== strpos($importline, '</wp:term>')) {
                        $in_term = false;
                        $this->terms[] = $this->process_term($term);
                        continue;
                    }
                    if ($in_term) {
                        $term .= $importline . "\n";
        ...
        function process_option($o) {
            return array(
                'option_id' => $this->get_tag($o, 'wp:option_id'),
                'option_name' => $this->get_tag($o, 'wp:option_name'),
                'option_value' => $this->get_tag($o, 'wp:option_value'),
                'autoload' => $this->get_tag($o, 'wp:autoload'),
            );
        }
