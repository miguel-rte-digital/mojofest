...
class EF_Excel_Importer_Field extends EF_Field_Base {
    ...
    public function display() {
        ...
        <section id="<?php echo $section_prefix . $this->id ?>" class="<?php echo $class ?>" <?php echo $style; ?>>
            ...
            <div class="import-excel-section">
                <label for="import-excel-tickets">Import Tickets (view <a href="<?php echo get_template_directory_uri(); ?>/misc/excel/tickets.xlsx" target="_blank">example</a>)</label>
                <p class="message"></p>
                <input id="" class="ef-upload-button button" type="button" value="Select" />
                <input type="hidden" class="ef-upload-button-hidden" value="" />
                <input id="" class="ef-import-button button" type="button" value="Import" data-type="ticket" />
            </div>
            <div class="import-excel-section">
                <label for="import-excel-exhibitors">Import Exhibitors (view <a href="<?php echo get_template_directory_uri(); ?>/misc/excel/exhibitors.xlsx" target="_blank">example</a>)</label>
                <p class="message"></p>
                <input id="" class="ef-upload-button button" type="button" value="Select" />
                <input type="hidden" class="ef-upload-button-hidden" value="" />
                <input id="" class="ef-import-button button" type="button" value="Import" data-type="exhibitor" />
            </div>
        </section>
        <script>
                            ...
                            case 'sponsor':
                                if (!empty($row[0])) {
                                    ...
                                    if (!is_wp_error($id) && $id != 0) {
                                        update_post_meta($id, 'sponsor_subtitle', $row[2]);
                                        update_post_meta($id, 'sponsor_link', $row[3]);
                                    }
                                }
                                break;
                            case 'ticket':
                                if (!empty($row[0])) {
                                    ...
                                    if (!is_wp_error($id) && $id != 0) {
                                        update_post_meta($id, 'ticket_price', $row[1]);
                                        update_post_meta($id, 'ticket_button_text', $row[2]);
                                        update_post_meta($id, 'ticket_button_link', $row[3]);
                                        update_post_meta($id, 'ticket_status', $row[4]);
                                        update_post_meta($id, 'ticket_features', $row[5]);
                                    }
                                }
                                break;
                            case 'exhibitor':
                                if (!empty($row[0])) {
                                    ...
                                    if (!is_wp_error($id) && $id != 0) {
                                        update_post_meta($id, 'exhibitor_info_1', $row[2]);
                                        update_post_meta($id, 'exhibitor_info_2', $row[3]);
                                        update_post_meta($id, 'exhibitor_info_3', $row[4]);
                                        update_post_meta($id, 'exhibitor_info_4', $row[5]);
                                        $categories = $row[6];
                                        if (!empty($categories)) {
                                            $terms_array = array();
                                            ...
                                            if (!empty($cat_array)) {
                                                foreach ($cat_array as $cat_name) {
                                                    $term = get_term_by('name', $cat_name, 'exhibitor-category');
                                                    if ($term === false) {
                                                        list($term_id, $term_taxonomy_id) = wp_insert_term($cat_name, 'exhibitor-category');
                                                        $terms_array[] = $term_id;
                                                    } else {
                                            ...
                                            if (!empty($terms_array)) {
                                                wp_set_object_terms($id, $terms_array, 'exhibitor-category');
                                            }
                                        }
                                    }
                                }
                            ...
                            case 'performer':
                                if (!empty($row[0])) {
                                    ...
                                    if (!is_wp_error($id) && $id != 0) {
                                        ...
                                        update_post_meta($id, 'speaker_function', $row[3]);
                                        update_post_meta($id, 'my_sessions_title', $row[4]);
                                        update_post_meta($id, 'my_sessions_subtitle', $row[5]);
                                        update_post_meta($id, 'linkedin_url', $row[6]);
                                        update_post_meta($id, 'facebook_url', $row[7]);
                                        update_post_meta($id, 'twitter_url', $row[8]);
                                    }
                                }
                                break;
                            case 'session':
                                if (!empty($row[0])) {
                                    ...
                                    if (!is_wp_error($id) && $id != 0) {
                                        update_post_meta($id, 'session_date', strtotime($row[2]));
                                        update_post_meta($id, 'session_time', $row[3]);
                                        update_post_meta($id, 'session_end_time', $row[4]);
                                        update_post_meta($id, 'session_speakers_title', $row[5]);
