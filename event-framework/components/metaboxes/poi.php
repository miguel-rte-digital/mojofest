...
function ef_metabox_poi($post) {
	...
	$poi_background_color = get_post_meta($post->ID, 'poi_background_color', true);
	?>
    <p>
	...
	<p>
		<label for="poi_background_color"><?php echo  __('Button background color', 'fudge'); ?></label><br />
		<input id="poi_background_color" type="text" name="poi_background_color" value="<?php echo $poi_background_color; ?>" placeholder="" class="poi_color"  />
	</p>
	<script type="text/javascript">
		jQuery(function() {
			jQuery('.poi_color').wpColorPicker();
		});
	</script>
    <?php
}
...
function ef_poi_save_post( $id ) {
	if( isset( $_POST['post_type'] ) && $_POST['post_type'] === 'poi' ) {
		...
		if (isset($_POST['poi_background_color']))
			update_post_meta($id, 'poi_background_color', $_POST['poi_background_color']);
		else
			delete_post_meta($id, 'poi_background_color');
